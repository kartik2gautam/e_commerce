...
export const RacingCategory = () => {
  ...
  const applyFilters = () => {
    ...
    axios.get(`http://127.0.0.1:8000/api/product/racing/filter-products/${filterform.min_price}/${filterform.max_price}/${filterform.publisher}/${filterform.sort_by}`)
    .then((response)=>{
  ...
  const handleSearch=(inputstring)=>{
    ...
    if(inputstring===''){
      axios.get(`http://127.0.0.1:8000/api/product/search/racing/`)
      .then((response)=>{
        setstring('')
        setFilteredProducts(response.data['data'])
      })
    ...
    axios.get(`http://127.0.0.1:8000/api/product/search/racing/${string}`)
          .then((response)=>{
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios
      .get(`http://127.0.0.1:8000/api/product/search/racing/${string}`)
      .then((response) => {
