...
export const AdventureCategory = () => {
  ...
  const [filterform, setfilterform] = useState({
    // creating formdata instance to post
    min_price: "0",
    max_price: "10000",
    publisher: "All",
  ...
  const applyFilters = () => {
    ...
    axios
      .get(
        `http://127.0.0.1:8000/api/product/adventure/filter-products/${filterform.min_price}/${filterform.max_price}/${filterform.publisher}/${filterform.sort_by}`
      )
      .then((response) => {
  ...
  const handleSearch = (inputstring) => {
    ...
    if (inputstring === "") {
      axios
        .get(`http://127.0.0.1:8000/api/product/search/adventure/`)
        .then((response) => {
          setstring("");
          setFilteredProducts(response.data["data"]);
        })
      ...
      axios
        .get(`http://127.0.0.1:8000/api/product/search/adventure/${string}`)
        .then((response) => {
        ...
        <div className="action-content-main mt-4">
          <div className="search-bar">
            <div className="searching-logo">
              <i
                className="fa-brands fa-searchengin fa-xl"
                style={{ color: "#ff0000" }}
              ></i>
            </div>
            <input
                  ...
                  <div className="form-group">
                    ...
                    >
                      <option
                        ...
                        className="black-option-action "
                      >
                        All
                      </option>
                      <option value="Ubisoft" className="black-option-action">
                        Ubisoft
                      </option>
                      <option
                        value="Focus Entertainment"
                        className="black-option-action"
                      >
                        Focus Entertainment
                      </option>
